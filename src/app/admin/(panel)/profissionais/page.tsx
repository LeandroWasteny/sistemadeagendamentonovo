...
export default async function ProfessionalsPage() {
  ...
  return (
    <section className="grid min-w-0 gap-6 lg:grid-cols-[minmax(300px,380px)_minmax(0,1fr)]">
      <form action={createProfessional} className="min-w-0 rounded-[20px] border border-white bg-white/95 p-5 shadow-xl shadow-blue-950/5">
        ...
        <div className="divide-y divide-blue-50">
          {professionals.map((professional) => (
            <div key={professional.id} className="p-5">
              <form action={updateProfessional} className="grid min-w-0 gap-3 xl:grid-cols-[minmax(0,1fr)_160px_minmax(0,1fr)_auto] xl:items-end">
              ...
              <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="flex w-full flex-wrap gap-2 sm:w-auto">
                  <form action={toggleProfessional} className="min-w-[132px] flex-1 sm:flex-none">
                    <input type="hidden" name="id" value={professional.id} />
                    <input type="hidden" name="active" value={String(!professional.active)} />
                    <Button className="w-full" variant="secondary">{professional.active ? "Desativar" : "Ativar"}</Button>
                  </form>
                  <form action={deleteProfessional} className="min-w-[132px] flex-1 sm:flex-none">
                    <input type="hidden" name="id" value={professional.id} />
